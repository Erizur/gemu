...
uint8_t GameBoy_read_mem(GameBoy* restrict gb, uint16_t addr) {
    gb->cycle_count++;
    ...
    BAIL("TODO: GameBoy_read_mem IE (addr = 0x%04X)", addr);
}
...
void GameBoy_write_io(GameBoy* const restrict gb, const uint16_t addr, const uint8_t value) {
    if (addr == 0xFF00) {
        // FF00 (joypad input)
        BAIL("I/O joypad input write (0x%04X, 0x%02X)", addr, value);
    } else if (addr <= 0xFF02) {
        // FF01-FF02 (serial transfer)
        BAIL("I/O serial transfer write (0x%04X, 0x%02X)", addr, value);
    } else if (addr >= 0xFF04 && addr <= 0xFF07) {
        // FF04-FF07 (timer and divider)
        BAIL("I/O timer and divider write (0x%04X, 0x%02X)", addr, value);
    } else if (addr == 0xFF0F) {
        // FF0F (interrupts)
        BAIL("I/O interrupts write (0x%04X, 0x%02X)", addr, value);
        ...
        SDL_Log("I/O boot ROM disable write (0x%04X, 0x%02X)", addr, value);
        if (value != 0) {
            gb->rom_enable = false;
        }
        BAIL("yay :D");
...
void GameBoy_write_mem(GameBoy* const restrict gb, const uint16_t addr, const uint8_t value) {
    ...
    if (addr <= 0x7FFF) {
        // 0000-7FFF (ROM bank)
        BAIL("TODO: GameBoy_write_mem ROM (addr = 0x%04X, 0x%02X)", addr, value);
    } else if (addr <= 0x9FFF) {
        // 8000-9FFF (VRAM)
        ...
        // FE00-FE9F (OAM)
        BAIL("TODO: GameBoy_write_mem OAM (addr = 0x%04X, 0x%02X)", addr, value);
    } else if (addr <= 0xFEFF) {
        // FEA0-FEFF (Not usable)
        BAIL("Tried to write into unusable memory (addr = 0x%04X, 0x%02X)", addr, value);
    } else if (addr <= 0xFF7F) {
        // FF00-FF7F I/O registers
        ...
        // FFFF (Interrupt Enable Register)
        BAIL("TODO: GameBoy_write_mem IE (addr = 0x%04X, 0x%02X)", addr, value);
    }
}
